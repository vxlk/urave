...
if (lastState == HIGH && currentState == LOW)       // button is pressed
    ...
    releasedTime = millis();
//TIME_2 = 15; //default time to run blowers 
    long pressDuration = releasedTime - pressedTime;
...
/*inside a while loop, add 10 to duty cycle for every short press of button 3 up to max of 255, subtract 10 for every short press of button 4 for min of 0, continuously check blower timer hasn't expired once every loop, check battery level once per loop, export
