...
uint8_t readSerialAndGetHeartRate() 
{
      ...
      if (buffer[i] == startBit) {
        ...
        for (auto x = i+1; x < 76; ++x) {
          if (buffer[x] == startBit) {
            return 0; // Invalid buffer sample ... will have to wait and go again
          }
        }
        auto offset = i;
        ...
        if (positionOfHeartRate + offset < BUFFER_SIZE) {
          ...
          return buffer[positionOfHeartRate + offset];
        }
      }
    }
  }
...
/**
 * Will return 0 if a heartrate is not in the buffer
 * */
uint8_t getHeartRateIfAvailable() {
    return readSerialAndGetHeartRate();
}
} //namespace bio
